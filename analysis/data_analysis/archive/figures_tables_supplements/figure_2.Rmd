...
```{r}
load("data/paper/enrichments/2023-03-17_domain_ORA_Nglyco.RData")
```
...
```{r}
load("data/paper/enrichments/2023-03-29_microheterogeneityER_string.RData")
...
```{r}
load("results/230213_glycositecorrelation_obsmod.RData")
```
...
site_corr_obsmod %>%
  ...
  ggpubr::stat_compare_means(label.y = 1,label.x = 1.4, paired = T) +
...
res_Oglyco$psm %>% 
  filter(species == "mouse\nbrain") %>% 
  distinct(Protein.ID, Protein.Start, Protein.End, glycan_type)%>% 
  mutate(N_glyco = ifelse(Protein.ID %in% unique(res_species$psm$Protein.ID), "N and O", "O")) %>% 
  group_by(glycan_type, N_glyco) %>%  
  mutate(bin_size = n()) %>% 
  ungroup() %>% 
  inner_join(mouse_psites %>% distinct(Protein.ID, psite_position), by = "Protein.ID")%>% 
  mutate(close = ifelse(Protein.Start-5 < psite_position &  psite_position < Protein.End+5, "psite closer than 5 aa", "no")) %>% 
  filter(!is.na(psite_position) & !is.na(glycan_type)) %>% 
  group_by(glycan_type,bin_size, close, N_glyco) %>% 
  summarise(a = n()) %>% 
  mutate(freq = a/bin_size*100)%>% 
  filter(close == "psite closer than 5 aa") %>% 
  ggplot(aes(x = glycan_type, y = freq, fill = N_glyco)) +
  geom_bar(stat = "identity", pos = "dodge") +
...
res_Oglyco$psm %>% 
  filter(species == "mouse\nbrain") %>% 
  distinct(Gene, Protein.ID, Protein.Start, Observed.Modifications) %>% 
  group_by(Gene, Protein.ID, Protein.Start) %>% 
  summarise(n_forms_sites = n_distinct(Observed.Modifications))%>%  
  ungroup() %>% 
  #mutate(grouping = ifelse(n_forms_sites ==1, "mono", "multi")) %>% 
  #group_by(grouping) %>% 
   mutate(#bin =as.character(ntile(n_forms_sites, n=3))) %>% 
         bin = ifelse(n_forms_sites ==1, "1", "2-4"),
         bin = ifelse(n_forms_sites > 4, "more than 4", bin)) %>% 
  #mutate(bin = factor(bin, levels = c("[1,2]", "(2,11]", "(11,738]"))) %>% 
  ungroup()%>% 
  group_by(bin) %>% 
  mutate(bin_size = n()) %>% 
  ungroup() %>% 
  left_join(mouse_psites %>% distinct(Protein.ID, psite_position), by = "Protein.ID")%>% 
  mutate(close = ifelse(Protein.Start-5 < psite_position &  psite_position < Protein.Start+5, "psite closer than 5 aa", "no")) %>% 
