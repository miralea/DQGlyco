...
data <- res_mouse_tissue_atlas$corr_per_site_per_mouse %>%  
 group_by(mouse, Protein.ID) %>%
  ...
  mutate(type = ifelse(mean_corr < 0.6691, "tissue specific", "not tissue specific"))%>% 
...
res_mouse_tissue_atlas$domain_ORA  %>% 
  ...
  filter(interpro_description %in% c("Plexin family", "Fibronectin type III superfamily", "Cadherin-like", "Immunoglobulin-like fold"))%>% 
  mutate(interpro_description = factor(interpro_description, levels = c("Plexin family", "Fibronectin type III superfamily", "Cadherin-like", "Immunoglobulin-like fold"))) %>% 
...
res_mouse_tissue_atlas$corr_per_site_per_mouse %>%
  ...
  tilted +
  labs(y="corelation across tissues")
```
...
hit_def <- res_mouse_tissue_atlas$corr_per_site_per_mouse%>%
  ...
  summarise(mean_corr = mean((value))) %>% 
  mutate(hit =  ifelse(mean_corr > 0.6691, "not tissue specific site", "tissue specific site"))%>% 
...
hit_def %>% 
  ...
  stat_summary(fun = median, geom = "crossbar", colour = "indianred") +
  ggpubr::stat_compare_means(label.y = 150, comparisons = list(c( "not tissue specific site", "tissue specific site"))) +
