...
psm_singleshot_combined <- res_singleshot$psm %>% 
  bind_rows(res_scanranges$psm %>%  filter(scan_range == "HeLa_1p5mg_glyco_700-2000")) %>% 
...
res_species$psm_total %>% 
  filter(!(dataset %in% c( "HEK_lf_PGC_frac","HeLa_lf_PGC_frac"))) %>% 
  mutate(glycosylation = as.character(glycosylation)) %>%
  # group_by(species, modification, data) %>% 
  # count()%>% 
  mutate(replicate = ifelse(grepl("human", species), dataset, "rep1")) %>% 
...
previos_anno_df <- read_csv("data/paper/revision1/comparison_to_datasets/merged_anno_glycosites_uniprot_psp.csv") %>%  mutate(origin = str_replace(origin, "out_data", "Potel et al."))
load("data/paper/revision1/processed/2024-01-08_combined_lf_data_human_and_mouse.RData")
```
...
res_species$psm %>% 
  ...
  ungroup() %>% 
  mutate(status = ifelse(grepl("psite|uniprot", or), "annoated", "not annotated")) %>% 
...
load("data/paper/revision1/results/2024-01-05_massrange_comparison.RData")
```
