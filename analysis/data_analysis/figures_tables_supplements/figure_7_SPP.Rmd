...
plot_data %>%
  ...
  geom_hline(yintercept = 0, colour = c("black")) +
  ggforce::geom_sina(size = 4, alpha= 0.5, stroke =NA, shape = 21) +
  stat_summary(aes(group = direction, colour = direction), fun = mean, geom = "crossbar", pos = "dodge",  linewidth = 0.4) +
  scale_colour_manual(values = c("down"= "dodgerblue4","no change" = "lightgrey","up" = "indianred")) +
  scale_fill_manual(values = c("down" = "dodgerblue4","no change" = "lightgrey","up" = "indianred")) +
  labs(y = "log2(NP40/SDS)", x = "N position in protein") +
  labs(subtitle = "Lrp1", fill = "change in\nmicrobiome data") +
